...
class Sudoku {
	...
	possibleCellOccupants(rowNum, columnNum) {
		//console.log(this.pCO[rowNum][columnNum])
	...
	computeAllPossibleCellOccupants() {
		this.pCO = this.pCO || []
		for (var i = 0; i < bigDim; i++) {
			if (this.pCO[i] === undefined) this.pCO.push([]);
			for (var j = 0; j < bigDim; j++) {
				const possibleVals = this.possibleCellOccupants(i, j)
				if (this.pCO[i][j] === undefined) {
					this.pCO[i].push(possibleVals)
				} else {
					this.pCO[i][j] = possibleVals
				}
			}
		}
	}
...
for (var i = 0; i < data.length; i++) {
	...
	//const sudoku = new Sudoku("000000509940000830000900602010700090000002050007006000000001900581429060069008400")
